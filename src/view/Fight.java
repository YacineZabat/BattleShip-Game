...
import controller.MultiPlayerFightController;
import controller.PcFightController;
import model.Battle;
...
public class Fight extends javax.swing.JFrame {
    ...
    Player player; 
    public Fight(Battle battle, int token){
    	this.battle = battle;  
    	if(token == 1)
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Mon champ de bataille :");
        ...
        jLabel4.setText("champ de bataille de l'adversaire :");

       

        jLabel5.setFont(new java.awt.Font("Bahnschrift", 0, 24)); // NOI18N
        jLabel5.setText("Turn : Player" + player.getId());
        ...
        for (int i = 0; i <10; i++)
        {
        	for (int j = 0; j <10; j++)
        	{
        		if(battle.getGameMode() == 1)
        			ennemy_field[i][j].addActionListener(new PcFightController(this,this.battle));
        		if(battle.getGameMode() == 2)
        			ennemy_field[i][j].addActionListener(new MultiPlayerFightController(this,this.battle, player.getId()));
        	}
        }
        
        // add placement_navire to my_field 
        
        try {
        	// diplay shots 
        	for (Coordinates shot : player.getBattlefield().getShots()) {
				my_field[shot.getI()][shot.getJ()].setBackground(Color.blue);
  			}
        	
        	// display ships
			for (Coordinates area : player.getBattlefield().getArea()) {
				 //Image img = ImageIO.read(getClass().getResource("resources/ship.png"));
				
				my_field[area.getI()][area.getJ()].setText("X");
				if(player.getBattlefield().getShots().contains(area))
					my_field[area.getI()][area.getJ()].setBackground(Color.red);
				else
				    my_field[area.getI()][area.getJ()].setBackground(Color.BLACK);
			}
			
			// display ennemy damage
			Player ennemy = null; 
			if(player.getId() == 1)
				ennemy = battle.getPlayer2();  
			if(player.getId() == 2)
				ennemy = battle.getPlayer1();  
			
			for (Coordinates coordinate : ennemy.getBattlefield().getShots()) {
				if(ennemy.getBattlefield().getArea().contains(coordinate))
					ennemy_field[coordinate.getI()][coordinate.getJ()].setBackground(Color.red); 
				else
					ennemy_field[coordinate.getI()][coordinate.getJ()].setBackground(Color.blue); 
			}
	...
	public JButton[][] getMy_field() {
		return my_field;
	}



	public JButton[][] getEnnemy_field() {
		return ennemy_field;
	}
